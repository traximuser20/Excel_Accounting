...
Sub CollectLedgerBalances()
    ...
    Set summaryWs = ThisWorkbook.Worksheets("Ledger Balance")
    On Error GoTo 0
    If summaryWs Is Nothing Then
        Set summaryWs = ThisWorkbook.Worksheets.Add
        summaryWs.Name = "Ledger Balance"
    End If

    ' Clear previous summary
    summaryWs.Cells.Clear
    summaryWs.Range("A1").Value = "Sheet Name"
    summaryWs.Range("B1").Value = "Total Ledger Balance"

    lastRow = 2

    ' Loop through each sheet
    For Each ws In ThisWorkbook.Worksheets
        If ws.Name <> summaryWs.Name And ws.Name <> "Master" And ws.Name <> "Stock Balance" Then
            
            Set headerCell = ws.Rows(2).Find(What:="Ledger Balance", LookIn:=xlValues, LookAt:=xlWhole)
    ...
    MsgBox "Ledger Balance summary updated!", vbInformation
End Sub
